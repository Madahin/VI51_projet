...
public class AntAgent extends Agent {
	...
	public void live1() {
		...
		/*for(Perceivable p : perceptions){
			if(((AntBody) body).faction == Faction.BlackAnt){
				System.out.println(p.getType().toString());
				if(p.getType().equals(PheromoneBody.class)){
					System.out.println(p.getPheromoneType());
				}
				System.out.println(isCarryingFood);
			}
		}*/
		
		// Check for pheromones on the same faction and or food
		for(Perceivable p : perceptions){
			
			if(p.getType().equals(PheromoneBody.class)){
				if(p.getFaction() == ((AntBody)body).faction){
					if(p.getPheromoneType() == PheromoneType.Food){
						pheromonesFood.add(p);
					}else if(p.getPheromoneType() == PheromoneType.Base){
						pheromonesBase.add(p);
					}
				}
			}else if(p.getType().equals(FoodPile.class)){
				foods.add(p);
			}else if(p.getType().equals(BlackBase.class) && ((AntBody)body).faction == Faction.BlackAnt){
				bases.add(p);
			}else if(p.getType().equals(RedBase.class) && ((AntBody)body).faction == Faction.RedAnt){
				bases.add(p);
			}
		}
		
		// we sort the pheromones food and base life > life.
		PheromoneComparator comparator = new PheromoneComparator();
		Collections.sort(pheromonesFood, comparator);
		Collections.sort(pheromonesBase, comparator);
		...
		if(isCarryingFood){
			...
			if(!bases.isEmpty()){
				...
				// if there an base pheromone
				if(!pheromonesBase.isEmpty()){
			...
			// If there no food
			if(foods.isEmpty()){
				// If there an food pheromones
				if(!pheromonesFood.isEmpty()){
					goToObject(pheromonesFood.get(0).getX(), pheromonesFood.get(0).getY());
				}else{
					wander();
				}
			}else{
				// Test if the food is on our feets
				int indFood = -1;
				int tmpind = 0;
				for(Perceivable p : foods){
					if(p.getX() == body.getX() && p.getY() == body.getY()){
						indFood = tmpind;
						break;
					}
